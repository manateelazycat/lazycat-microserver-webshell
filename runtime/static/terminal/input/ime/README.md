...
聚焦前只准备必要的可编辑几何，完整定位合并到后续帧；失焦和销毁使旧定位回调失效，composition 更新仍及时处理。`isKeyboardClaimed(event)` 是只读查询，selection、TUI、mouse 可共同读取同一次判定。`shouldPreserveInputFocus(session)` 仅用于快捷栏维持输入，不代表系统键盘已显示。
...
临时 `keyboard_diagnostics.js` 在触摸设备记录 `[键盘诊断 kbd-v3-native-tap-…]`：初始化、document 与 pane 触摸及兼容鼠标事件、双击判定、focus/blur 和延迟视口快照。通过注入的日志入口使用 `retainWhenDisabled`，即使错误日志窗口未打开也保留记录；复现后打开窗口复制即可。同步路径只收集状态，布局测量在后续 timer 中执行，事件每 400ms 批量写入（每批最多 100 条，溢出计数），继续遵守错误日志原有 200 条批次保留上限。页面销毁清理监听器及定时器。不会采集输入值、终端输出或选区文本；DOM focus 不作为原生键盘出现的证据。对照首次双击失败和其他按钮操作后恢复时应保留同一次页面加载的完整日志，不要中途清空。

v2 的 window.focus 验证在 Android 真机正常返回但未恢复 document 焦点，已撤下。当前 v3 仅在 document 未聚焦、未识别为双击、无滚动且事件尚未取消时，尝试保留单次轻触的默认行为；Ghostty 包装器只结束滚动状态，不取消该 touchend。shell 捕获对应的兼容 mousedown/mouseup/click，只 stopImmediatePropagation，不 preventDefault，避免 Ghostty 单击聚焦或重复鼠标输入，同时允许浏览器默认聚焦。匹配限于同终端、16px 内、1000ms 内的可信左键事件，排除明确的物理鼠标来源，并在物理鼠标 pointerdown 或匹配 click 后清理；该窗口仅用于兼容鼠标事件匹配，不是键盘许可。记录 tap.page-default-offered/host/mouse，结合 document.touchend.bubble 的 prevented 和 window.focus 判断实际放行效果。其他 selection/TUI 处理器仍可能取消事件，此轮不绕过其业务逻辑。保持有效双击、滚动、初始化和 textarea 定位的既有处理。2026-09-08 用户在 Android 客户端壳与 iOS 客户端手测确认首次双击可以展开键盘；两端日志均记录到第一下默认触摸之后 document 获得焦点，第二下 textarea 聚焦并出现键盘对应的视口收缩。该结果只覆盖本次复现场景，旧设备滚动后的偶发失败、iOS 偶发整页上移及多会话性能问题仍未验收。
