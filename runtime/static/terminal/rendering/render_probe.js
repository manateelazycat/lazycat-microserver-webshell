  ...
  return Object.freeze({
    capture(session, { includePixels = false } = {}) {
      ...
      const state = {
        ...
        connectionState: session.shellEl?.dataset?.connection || "unknown",
        transport: { channel: session.connectionChannel, channelGeneration: session.connectionChannelGeneration,
          socketReadyState: session.socket?.readyState ?? null, pendingConnect: session.pendingConnect === true,
          unifiedConnectPending: session.unifiedConnectPending === true, agentPreparing: session.agentPreparing === true },
        replay: { phase: session.replayController?.phase || "unknown", retryPaused: session.replayRetryPaused === true,
          failureAttempts: session.replayFailureAttempts || 0, lastFailure: String(session.lastReplayFailureReason || "").slice(0, 8192) },
        ...
        holdDiagnostics: presentation?.holdDiagnostics?.(session),
        resizeDiagnostics: resize?.snapshot?.(session),
