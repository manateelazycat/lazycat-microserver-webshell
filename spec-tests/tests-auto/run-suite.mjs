...
import { readConfig, readEnvironment, validateConfig } from '../environment/config.mjs';
import { redactDiagnosticText } from './artifact-redaction.mjs';
import { prepareFrontend, frontendEnvironment, frontendEvidence, verifyFrontend } from '../environment/frontend-build.mjs';
import { reportStandaloneRunTime } from '../environment/run-timing.mjs';
...
const event = (item, status, duration_seconds) => {
  ...
  else process.stderr.write(`[tests-auto] ${status.toUpperCase()} ${item.test_id}${duration_seconds === undefined ? '' : ` (${duration_seconds}s)`}\n`);
};
try {
  ...
  const folders = (await fs.readdir(here, {withFileTypes:true})).filter(entry => entry.isDirectory() && /^\d{2}-/.test(entry.name)).map(entry => entry.name).sort();
  const discovered = [];
  for (const folder of folders) {
    try { await fs.access(path.join(here,folder,'test.mjs')); discovered.push({test_id:`tests-auto/${folder}/test.mjs`}); } catch {}
  }
  const selected = values['--scenarios-file']
    ? JSON.parse(await fs.readFile(values['--scenarios-file'],'utf8')).scenarios
    : discovered.filter(item => !values['--case'] || item.test_id === `tests-auto/${values['--case']}/test.mjs`);
  ...
  if (flags.has('--dry-run') || environment.TESTS_AUTO_DRY_RUN === '1') {
    ...
    const reportRoot = path.resolve(values['--artifacts-dir'] || path.join(root,'artifacts/suites',randomUUID()));
    await fs.mkdir(reportRoot,{recursive:true});
    await fs.writeFile(path.join(reportRoot,'frontend.json'),JSON.stringify(frontendEvidence(frontend),null,2));
    const results = [];
    for (const item of selected) {
      const started = performance.now(), caseName = item.test_id.split('/')[1];
      ...
      if (interrupted) result = {...result,status:'skipped',error:'Suite interrupted before this module started'};
        ...
        if (caseName === '04-terminal-viewport' && !env.WEBSHELL_MOBILE_USER_AGENT) env.WEBSHELL_MOBILE_USER_AGENT = 'Mozilla/5.0 (iPhone; CPU iPhone OS 18_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/18.0 Mobile/15E148 Safari/604.1';
        let timedOut = false, diagnostic = '';
        const child = spawn(process.execPath,[path.join(here,'run-playwright.mjs'),path.join(root,item.test_id),'--artifacts-dir',directory,'--timeout',String(timeout)],{cwd:root,env,stdio:['ignore','pipe','pipe']});
        currentChild = child;
