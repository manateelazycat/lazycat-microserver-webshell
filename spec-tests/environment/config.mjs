...
export async function readEnvironment(overrides = {}) {
  ...
  const filename = overrides.envFile || env.WEBSHELL_TEST_ENV_FILE || path.join(projectRoot, "tests-auto/.env");
  let contents = "";
...
export async function readConfig(overrides = {}) {
  ...
  return {
    ...
    moduleRoot: env.PLAYWRIGHT_NODE_PATH || path.join(projectRoot, "tests-auto/node_modules"),
