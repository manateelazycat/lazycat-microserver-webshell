...
4. 指定普通实例不可用时，选择返回列表中的第一个 `running` 实例，去掉旧 tab 参数，并把请求实例和实际选择写入事件日志。没有可用实例时失败。
5. 客户端历史恢复模块 17 必须使用真实、在线、获授权的 `client:` 目标；可通过 `WEBSHELL_CLIENT_TEST_URL` 单独指定。它不能回退到普通容器冒充客户端验收。
